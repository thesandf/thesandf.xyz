        ...
        if (tokenSupplyAddr < 4200000000000000001) {
            if (selfPesto > (4200000000000000001 - tokenSupplyAddr) && selfPesto >= 4200000000000000001) {
                ...
                IUniswapV2Router02(UniswapV2Router02).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    amtIn, 0, _path(PestoTheBabyKingPenguin, weth9), address(this), block.timestamp + 1
                );

                uint256 tokenSupplyAddr2 =
                    IPestoTheBabyKingPenguin(PestoTheBabyKingPenguin).balanceOf(PestoTheBabyKingPenguin);
                if (tokenSupplyAddr2 < 4200000000000000001) {
                    ...
                    IUniswapV2Router02(UniswapV2Router02).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        remainPesto, 0, _path(PestoTheBabyKingPenguin, weth9), address(this), block.timestamp + 1
                    );
